...
class PythiaVAE(torch.nn.Module):
    ...
    **Decoder**  – frozen; receives a *single* latent‑embedding token `z` as prefix and must
    ...
    def __init__(self, model_name: str = "EleutherAI/pythia-160m", latent_size: int = 768):
        ...
        self.to_logvar = torch.nn.Sequential(
            torch.nn.Linear(self.hidden_size, 512),
            torch.nn.BatchNorm1d(512),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.3),
        ...
        self.latent_to_hidden = torch.nn.Sequential(
            torch.nn.Linear(self.latent_size, 512),
            torch.nn.BatchNorm1d(512),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.3),

            torch.nn.Linear(512, 368),
            torch.nn.BatchNorm1d(368),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.3),

            torch.nn.Linear(368, 256),
            torch.nn.BatchNorm1d(256),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.3),
...
if __name__ == "__main__":
    # Initialize W&B
    wandb.init(project='vae_training_2')
